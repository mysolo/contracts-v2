...
contract Index is AIndex {
    ...
    function getComposition()
        external
        view
        returns (IndexComposition[] memory)
    {
        return composition;
    }
    ...
    function purchaseIndex(
        ...
        if (
            true ||
            address(sellToken) == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE
        ) {
    ...
    function _purchaseUnderlyingAssets(
        ...
        for (uint256 i = 0; i < tokenOrders.length; i++) {
            ...
            uint256 amountBought = tokenExchanger.executeTrade(
                ...
                uint256 indexAmountPurchased /* todo rename */
             = (amountBought * 1e18) / composition[i].amount;
