...
library PancakeswapUtilities {
    ...
    function buyToken(
        ...
        (uint256 reservesA, uint256 reservesB) = getReservesOrdered(
            pair,
            tokenToSpend,
            tokenToBuy
        );
        uint256 amountInMin = pancakeRouter.getAmountIn(
            amountOut,
            reservesA,
            reservesB
        );
        IERC20(tokenToSpend).approve(address(pancakeRouter), amountInMin);
        address[] memory path = new address[](2);
        path[0] = tokenToSpend;
        path[1] = tokenToBuy;

        uint256[] memory amounts = pancakeRouter.swapTokensForExactTokens(
            amountOut,
            amountInMin,
            path,
            account,
    ...
    function getReservesOrdered(
        IUniswapV2Pair pair,
        address tokenFirst,
        address tokenSecond
    ) public view returns (uint256, uint256) {
        (address token0, ) = PancakeswapUtilities.sortTokens(
            tokenFirst,
            tokenSecond
        );
        (uint256 tokenAReserve, uint256 tokenBReserve, ) = pair.getReserves();
        return
            address(tokenFirst) == token0
                ? (tokenAReserve, tokenBReserve)
                : (tokenBReserve, tokenAReserve);
    }
}
